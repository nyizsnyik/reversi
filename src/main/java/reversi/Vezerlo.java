...
public class Vezerlo {
	...
	/**
	 * Létrehoz egy {@code Vezerlo} objektumot kapott tábla objektum felhasználásával.
	 * @param aktJatekos kezdő játékos.
	 * @param table játék táblája.
	 */
	 ...
	 * @return a két játékos pontjainak külömbségével ez alapján lehet eldönteni ki nyert.
	 */
	public int Play() {
		logger.debug("Play metódus meghíva.");
		Scanner sc = new Scanner(System.in);
		while (!(vegeE(this.table.getTable()))) {
			TreeSet<Kordinata> ts = lehetLepesek(aktJatekos, this.table.getTable());
			table.lehetsegesLesesek(ts);
			System.out.println(table); logger.debug("Tábla kiírva.");
			table.lehetsegesLesesekEltuntet();
			System.out.println("1. játékos pontjai: " + pontok(1, this.table.getTable()) + "\n2. játékos pontjai: " + pontok(-1, this.table.getTable()) + "\n" + jatekos(aktJatekos) + ". játékos következik.");
			logger.debug("Játékosok pontjai, aktuális játékos kiírva.");
			...
			try {
				Kordinata k = new Kordinata(sc2.nextInt() - 1,sc2.nextInt() - 1);
				...
				if (!ts.isEmpty()) {
					...
					if (ts.contains(k)) {
						...
						table.lep(k, aktJatekos);
						logger.debug("Lépés elvégezve.");
						if (!lehetLepesek(aktJatekos * -1, table.getTable()).isEmpty()) {
				...
				System.out.println("\nKérem két darab egész számot adjon meg szóközzel leválasztva.");}
			sc2.close();
			logger.debug("sc2 bezárása megtörtént.");}
		sc.close();
		logger.debug("sc bezárása megtörtént.");
		System.out.println(this.table + "\n1. játékos pontjai: " + pontok(1, this.table.getTable()) + "\n2. játékos pontjai: " + pontok(-1, this.table.getTable()));
		logger.debug("Játékosok pontjainak kiszámítása és végeredmény elészítése.");
		return pontok(1, this.table.getTable()) - pontok(-1, this.table.getTable());
	}
	
	/**
	 * A megkapott táblát átvizsgálja a kapott játékos szemszögéből és visszadja  milyen lépési lehetőségei vannak.
	 * @param aktJatekos melyik játékos szemszögéből vizsgáljuk a <code>table</code> t.
	 * @param table az a tábla amit vizsgál a metódus.
	 ...
	 * @param jatekos melyik játékos pontjaira vagyunk kíványcsiak.
	 * @param table a tabla amelyikben meg kell számolni az adott játékos pontjait.
	 ...
	 * @param jatekos melyik játékos kódját akarjul lefordítani.
	 * @return ha a <code>jatekos</code> egyenő 1-el akkor 1-el tér vissza ha <code>jatekos</code> egyenlő -1 akkor 2-vel tér vissza.
	 */
	 ...
	 * @param table az a tábla amit meg kell vizsgálni.
